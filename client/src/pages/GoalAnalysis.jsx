...
const SaveCard = ({ label, value }) => (
  <div className="bg-white border border-gray-100 rounded-2xl p-4 text-center shadow-sm">
    <p className="text-base font-bold font-mono text-gray-900 leading-tight">
      {value}
    </p>
    ...
    <div className="p-5 max-w-2xl mx-auto font-sans space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <div className="flex justify-between items-start mb-4">
          ...
          {feas && (
            <span
              className={`flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full border ${fStyle.bg} ${fStyle.text} ${fStyle.border}`}
            >
        ...
        <div className="grid grid-cols-3 gap-2 mt-4">
          ...
          <MetaCard
            label="Months Left"
            value={
              analysis?.analysis?.monthsLeft
                ? `${analysis.analysis.monthsLeft} mo`
                : "—"
            }
          />
          <MetaCard
            label="Remaining"
            value={`₹${(goal.targetAmount - goal.savedAmount).toLocaleString("en-IN")}`}
            mono
          />
        </div>
      </div>
      ...
      {analysis && (
        <div className="space-y-4">
          ...
          <div className="rounded-2xl bg-blue-50 px-5 py-4">
            <p className="text-[10px] text-gray-800 uppercase tracking-widest mb-2">
              AI Insight
            </p>
            <p className="text-sm text-black font-semibold italic leading-relaxed">
              "{analysis.summary}"
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="space-y-3">
              {analysis.professionalTips.map(
                ({ category, icon, title, tip }, i) => (
                  <div
                    key={i}
                    className="flex gap-3 p-3 rounded-xl bg-green-50 border border-gray-100"
                  >
          ...
          {analysis.instruments?.length > 0 && (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="space-y-2">
                {analysis.instruments.map(
                  ({ name, expectedReturn, risk, suitable }, i) => {
                    const rs = RISK_COLOR[risk] ?? RISK_COLOR.Medium;
                    return (
                      <div
                        key={i}
                        className={`flex items-center justify-between p-3 rounded-xl border ${suitable ? "bg-white border-gray-100" : "bg-gray-50 border-gray-100 opacity-50"}`}
                      >
                        ...
                        <div className="flex items-center gap-2">
                          <div className="w-16">
                            <div className="h-1 bg-gray-100 rounded-full overflow-hidden">
                              <div
                                className={`h-1 ${rs.bar} rounded-full`}
                                style={{ width: RISK_BAR_WIDTH[risk] }}
                              />
                            </div>
                          </div>
                          <span
                            className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${rs.bg} ${rs.text}`}
                          >
                            {risk}
                          </span>
                        </div>
